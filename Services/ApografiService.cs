...
namespace HR.Apografi.Services
{
    public class ApografiService
    {
        ...
        {
            var cacheKey = CacheKeys.Organizations; // ftiakse ena cache klidi
            if (_cache.TryGetValue<List<OrganizationLite>>(cacheKey, out var cached)) // psaxe na vreis stin cache ean yparxei to kleidi, ean yparxei gyrna true kai sto cached tin timi pou brikes
            {
                return cached;
            }
            ...
            try
            {
                await _lockOrganizations.WaitAsync();// ean edw erthoun XWRIS na exei cache, tha perasei parakatw mono 1as kai oi ypoloipoi tha perimenoun edw na teleiwsei o prwtos.
                if (_cache.TryGetValue(cacheKey, out cached))// o prwtos tha psaxei edw stin cache ksana, den tha vrei kai tha paei parakatw. oi epomenoi omws, tha psaxoun stin cache, giati o prwtos
                    // exei gemisei tin cache, ara den exei noima na pane parakatw kai na ksanakanoun ta idia.
                {
                    return cached;
                }

                var result = await _httpClient.GetStringAsync("public/organizations"); // edw kaleis tin apografi api me to route tou endpoint tous. perneis apotelesma ena json keimeno.
                var apografiResponse = JsonSerializer.Deserialize<ApografiResponse<List<OrganizationLite>>>(result, _serializerOptions);// edw kaneis to json keymeno MONTELA

                _cache.Set(cacheKey, apografiResponse.Data, TimeSpan.FromDays(1));// edw apothikeyeis ta montela stin cache, wste oi epomenoi na mhn xreiastei na kalesoun tin apografi gia ta idia dedomena
                return apografiResponse.Data;// edw epistrefeis ta dedomena se auton pou kalese tin methodo.
            }
            finally
            {
                _lockOrganizations.Release();//edw afineis to kleidi eleuthero, auto poy kleidwses olous tous allous plin ton 1o stin grammi 43. kathe fora mono enas tha pairnei to kleidi. oi ypoloipoi tha perimenoun.
            }
        }
